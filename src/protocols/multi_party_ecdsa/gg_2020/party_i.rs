...
impl Keys {
    pub fn create(index: usize) -> Self {
        ...
        //let u = Scalar::<Secp256k1>::from_bigint(&BigInt::from_str_radix("53148706113055400461778566684754052970592521397982588090730450887828980789535",10).unwrap());
        println!("u {}",u.to_bigint());
    ...
    pub fn create_hd_prime(index: usize) -> Self {
        let u = Scalar::<Secp256k1>::random();
        let y = Point::generator() * &u;
...
pub fn verify(sig: &SignatureRecid, y: &Point<Secp256k1>, message: &BigInt) -> Result<(), Error> {
    ...
    let b : [u8 ; 64] = y.to_bytes(false).deref()[1..].try_into().unwrap();
    let digest = Keccak256::digest(b);
    //hasher.update(b); // 跳过前缀字节 0x04
    //let result = hasher.finalize();

    // 取 Keccak-256 哈希的后 20 字节，这就是以太坊地址的字节表示
    let address_bytes = &digest[12..];

    // 将字节表示的地址转换为十六进制字符串
    println!("pk 0x{}", hex::encode(address_bytes));

    // // 将 r、s 和 v 转换为字节序列
    // let r_bytes = sig.r.to_bytes().deref().to_owned();
    // let s_bytes = sig.s.to_bytes().deref().to_owned();
    // //let v_byte = vec![sig.recid + 27];

    // // 将所有部分连接起来
    // let mut combined = Vec::new();
    // combined.extend_from_slice(message.to_bytes().deref());
    // combined.extend_from_slice(&r_bytes);
    // combined.extend_from_slice(&s_bytes);
    // combined.extend_from_slice(&v_byte);

    // // 对连接的数据再进行一次哈希
    // // let mut hasher = Keccak256::new();
    // // hasher.update(&hex::encode(combined).as_bytes()); 
    // // let result = hasher.finalize();
    // println!("sig 0x{}", hex::encode(combined));
